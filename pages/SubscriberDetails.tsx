...
const DetailCard = ({ title, children, defaultOpen = true, icon: Icon }: { title: string, children?: React.ReactNode, defaultOpen?: boolean, icon?: any }) => {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6 transition-all duration-200">
            ...
            {isOpen && (
                <div className="px-6 pb-8 border-t border-gray-100 pt-6 animate-in fade-in slide-in-from-top-1 duration-200">
                    {children}
                </div>
            )}
        </div>
...
export const SubscriberDetails: React.FC = () => {
  ...
  if (!data) return <div className="p-8 text-center text-gray-500">Subscriber not found.</div>;
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
              <div className="relative">
                  ...
                  <span className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-white ${chitProgress.dynamicStatus === 'Active' ? 'bg-green-500' : 'bg-gray-400'}`}></span>
              </div>
              <div>
